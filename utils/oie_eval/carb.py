'''
Usage:
   carb.py --gold=GOLD_OIE --out=OUTPUT_FILE (--openiefive=OPENIE5 | --stanford=STANFORD_OIE | --ollie=OLLIE_OIE |--reverb=REVERB_OIE | --clausie=CLAUSIE_OIE | --openiefour=OPENIEFOUR_OIE | --props=PROPS_OIE | --tabbed=TABBED_OIE | --benchmarkGold=BENCHMARK_GOLD | --allennlp=ALLENNLP_OIE ) [--exactMatch | --predMatch | --lexicalMatch | --binaryMatch | --simpleMatch | --strictMatch] [--error-file=ERROR_FILE] [--binary]
Options:
...
from .oie_readers.seqReader import SeqReader
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Usage for oie benchmark.")
    parser.add_argument('--task', type=str, help="Path to config file.")
    parser.add_argument('--gold_path', type=str, help="Path to gold file.")
    parser.add_argument('--out', type=str, help="Path to benchmark output file.")
    parser.add_argument('--outfile_path', type=str, help="Path to model output file.")
    parser.add_argument('--matcher', type=str, help="chose matcher.", default=None)
    parser.add_argument('--error_file', type=str, help="chose matcher.", default='./log')
    ...
    if args.task == 'seq':
        predicted = SeqReader()
        predicted.read(args.outfile_path)

    # elif args.task == 'ure':
    #     predicted = UnireReader()
    #     predicted.read(args.outfile_path)
    if args.matcher == 'exactMatch':
        matchingFunc = Matcher.argMatch

    elif args.matcher == '--predMatch':
        matchingFunc = Matcher.predMatch

    elif args.matcher == '--argMatch':
        matchingFunc = Matcher.argMatch
    else:
        # matchingFunc = Matcher.lexicalMatch
        matchingFunc = Matcher.binary_linient_tuple_match
    b = Benchmark(args.gold_path)
