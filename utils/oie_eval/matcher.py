...
class Matcher:
    ...
    def tuple_match(ref, ex, ignoreStopwords, ignoreCase):
        ...
        # the predicate must match first
        if matching_words == 0:
        ...
        for i in range(len(ref.args)):
            ...
            recall[1] += len(gold_words)
            # pass the additional args of the gold for the predicted extraction
            if len(ex.args) <= i:
                if i<2:
                    return False
                else:
            ...
            # for first two argument spans, must have matching words
    ...
    # binary: an extraction' args are splited into two spans [arg1, arg2:] 
    @staticmethod
    def binary_linient_tuple_match(ref, ex, ignoreStopwords, ignoreCase):
        if len(ref.args)>=2:
            r = copy(ref)
